...
public class CreditPage {
    ...
    public void emptyFieldNotification() {
        proceedButton.click();
        fieldCard.$(".input__sub").shouldBe(visible).shouldHave(ownText("Неверный форма"));
        fieldMonth.$(".input__sub").shouldBe(visible).shouldHave(ownText("Неверный форма"));
        fieldYear.$(".input__sub").shouldBe(visible).shouldHave(ownText("Неверный форма"));
        fieldCardOwner.$(".input__sub").shouldBe(visible).shouldHave(ownText("Поле обязательно для заполнения"));
        fieldCvc.$(".input__sub").shouldBe(visible).shouldHave(ownText("Неверный форма"));
    }
}
